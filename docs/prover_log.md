...
2026-07-21 | `triangular_faces_diagonal_ne` | Claude Sonnet 5 | **statement corrected** (not a proof attempt) | Author pointed out that `edge_bound_no_four_cycles` — the only real caller, two levels up — already has `hn : 5 ≤ Fintype.card V` in scope; it just never got passed down through `triangular_faces_edge_disjoint` (which didn't accept it) into `triangular_faces_diagonal_ne` (ditto). So the fix isn't a new mathematical assumption, it's plumbing an already-true fact through. Added `hn` to both lemmas' signatures and threaded it through the one call chain (`edge_bound_no_four_cycles` → `triangular_faces_edge_disjoint` → `triangular_faces_diagonal_ne`). `lake build` green (1315 jobs), still `sorry`. Explicitly still open: whether `n ≥ 5` is actually *sufficient* to prove the (now presumably true) statement, versus merely ruling out the `n=3` counterexample — next proof attempt needs to find out.
