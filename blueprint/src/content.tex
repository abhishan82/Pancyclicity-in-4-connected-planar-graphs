...
\begin{definition}[Graph family $G_k$]
  ...
  Not currently linked to a Lean formalization: the prior axiomatized-data
  encoding (\texttt{Gk}) was quarantined (moved to
  \texttt{docs/deferred/GraphFamily.lean.disabled}) since an axiomatized
  graph with axiomatized properties risks an inconsistent environment. See
  \texttt{docs/graphfamily\_options.md} for the plan to reinstate it via a
  concrete construction or a hypothesis-bundling structure.
\end{definition}
